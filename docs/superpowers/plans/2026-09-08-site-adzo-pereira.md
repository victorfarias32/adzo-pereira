...
**Tech Stack:** Astro 5, TypeScript, Vitest, linkedom (asserções sobre o HTML gerado), `astro:assets`/sharp (otimização de imagem), `@astrojs/sitemap`, Vercel.
...
- Produces: URL pública dos 3 protótipos na Vercel
...
```bash
git add -A
git commit -m "chore: configuração da Vercel e ajustes de performance e acessibilidade"
git push
npx --yes vercel@latest --prod --yes
```

Expected: a Vercel imprime a URL de produção. Anote-a — a Task 11 precisa dela.

- [ ] **Step 7: Conferir a URL pública**

Abra `<url>/prototipo/a`, `/b`, `/c` e `/`. Clique um CTA de WhatsApp e confirme que o app abre com a mensagem correta já escrita.
...
7. Apontar o domínio próprio na Vercel e atualizar `site` em `astro.config.mjs`.
